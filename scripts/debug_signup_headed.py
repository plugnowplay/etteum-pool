...
with Camoufox(headless=False, humanize=True, geoip=True, proxy=proxy) as browser:
    ...
    if btn:
        ...
        # Screenshot
        page.screenshot(path="/tmp/debug_headed.png", full_page=True)
        print("Screenshot: /tmp/debug_headed.png", flush=True)
    else:
        print("No submit button found", flush=True)
        page.screenshot(path="/tmp/debug_headed_nobtn.png", full_page=True)
