...
accountsRouter.post("/:id/upstream-models", async (c) => {
  ...
  try {
    ...
    const prefixMap: Record<string, string> = { qoder: "qd", "grok-cli": "gcli", codebuddy: "cb", "codebuddy-china": "cbcn", codex: "cx", canva: "cv", kiro: "k" };
...
accountsRouter.post("/", async (c) => {
  const body = await c.req.json<{
    provider: "kiro" | "codebuddy" | "codebuddy-china" | "codebuddy-github" | "canva" | "codex" | "qoder" | "grok" | "grok-cli";
    accessToken?: string;
    refreshToken?: string;
    idToken?: string;
    email?: string;
  ...
  // ── YouMind retired (2026-08-29): provider removed from the registry. Pasted
  // sk-ym-... keys are rejected with a clear message instead of a silent 500.
